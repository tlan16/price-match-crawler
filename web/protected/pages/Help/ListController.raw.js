	...
	,getHelpDiv: function() {
		...
		tmp.newDiv
			.insert({'bottom': new Element('h2', {'id': 'windows-setting'}).update('Windows') 
				...
				.insert({'bottom': new Element('img', {'data-src': '/media/windows-driver-setting.png'}) })
				
				.insert({'bottom': new Element('h3', {'id': 'windows-driver-setting'}).update('Browser Setting<br/>') })
				.insert({'bottom': new Element('img', {'data-src': '/media/windows-chrome-setting.1.png'}) })
				.insert({'bottom': new Element('i', {'class': 'fa fa-arrow-right'}).setStyle('margin: 0 10px;') })
				.insert({'bottom': new Element('img', {'data-src': '/media/windows-chrome-setting.2.png'}) })
			})
		;
		tmp.newDiv.getElementsBySelector('img').each(function(img){img.addClassName('lazyload')});
		return tmp.newDiv;
	}
});