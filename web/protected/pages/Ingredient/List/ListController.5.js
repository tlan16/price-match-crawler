...
PageJs.prototype = Object.extend(new CRUDPageJs(), {
	...
	,_getResultRow: function(row, isTitle) {
		...
		tmp.row = new Element('span', {'class': 'row'})
			...
			.insert({'bottom': new Element(tmp.tag, {'class': 'name col-sm-6 col-xs-12'}).update(row.name) })
			.insert({'bottom': new Element(tmp.tag, {'class': 'description col-sm-2 col-xs-12'}).update(row.description) })
			.insert({'bottom': new Element(tmp.tag, {'class': 'allergents col-sm-2 col-xs-12'}).update(tmp.isTitle === true ? 'Allgergents' : tmp.me._getNamesString(row.infos.allergents)) })
