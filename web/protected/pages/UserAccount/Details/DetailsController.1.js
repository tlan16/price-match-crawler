...
PageJs.prototype = Object.extend(new DetailsPageJs(), {
	...
	,_addComboRow: function(combo, container) {
		...
		tmp.container
			.insert({'bottom': new Element('div', {'class': 'combo col-xs-12', 'combo_id': (tmp.combo ? tmp.combo.id : 'new'), 'active': (tmp.combo ? tmp.combo.active : true) })
				.insert({'bottom': new Element('div', {'class': 'row '})
					.insert({'bottom': tmp.store = new Element('div', {'class': 'store col-md-7 col-sm-6 col-xs-12'}) })
					.insert({'bottom': tmp.role = new Element('div', {'class': 'role col-md-4 col-sm-5  col-xs-12'}) })
		...
		tmp.storeSelect2Options = {
			...
			ajax: {
				...
				,data: function (params) {
					return {"searchTxt": 'name like ?', 'searchParams': ['%' + params + '%'], 'entityName': 'Store', 'pageNo': 1};
				}
		...
		tmp.me
			._getSelect2Div('Store', 'store', ((tmp.combo && tmp.combo.store) ? {'id': tmp.combo.store.id, 'text': tmp.combo.store.name, 'data': tmp.combo.store} : null), tmp.store, ' ', true, tmp.storeSelect2Options)
